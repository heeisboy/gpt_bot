...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.commands = [
            ...
            BotCommand(command='reset',
                       description='Сбросить разговор. Опционально передавать высокоуровневые инструкции '
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_text = 'Я бот ChatGPT, можете поговорить со мной!' + \
                    '\n\n' + \
                    '\n'.join(commands) + \
                    '\n\n' + \
                    'Отправьте мне голосовое сообщение или файл, и я расшифрую его для вас!' + \
                    '\n\n' + \
                    "Открытый исходный код на https://github.com/n3d1117/chatgpt-telegram-bot"
    ...
    async def stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await self.is_allowed(update, context):
            logging.warning(f'Пользователь {update.message.from_user.name} (id: {update.message.from_user.id}) '
                            f'не может запрашивать статистику использования')
        ...
        text_today = f"*Использование сегодня:*\n" + \
                     f"{tokens_today} используются токены чата.\n" + \
                     f"{images_today} сгенерированные изображения.\n" + \
                     f"{transcribe_durations[0]} минут и {transcribe_durations[1]} секунды записаны.\n" + \
                     f"💰 На общую сумму ${cost_today:.2f}\n" + \
                     f"----------------------------\n"
        text_month = f"*Использование в этом месяце:*\n" + \
                     f"{tokens_month} используются токены чата.\n" + \
                     f"{images_month} сгенерированные изображения.\n" + \
                     f"{transcribe_durations[2]} минут и  {transcribe_durations[3]} секунды записаны.\n" + \
                     f"💰 На общую сумму ${cost_month:.2f}"
        # text_budget filled with conditional content
        text_budget = "\n\n"
        if budget < float('inf'):
            text_budget += f"Остаток бюджета составляет ${budget:.2f} в этом месяце.\n"
        # add OpenAI account information for admin request
        if self.is_admin(update):
            text_budget += f"С вашей учетной записи OpenAI был выставлен счет ${self.openai.get_billing_current_month():.2f} в этом месяце."
    ...
    async def resend(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id not in self.last_message:
            ...
            await context.bot.send_message(chat_id=chat_id, text="Вам нечего повторно отправить")
            return

        # Update message text, clear self.last_message and send the request to prompt
        logging.info(f'Повторная отправка последнего приглашения от пользователя: {update.message.from_user.name} '
    ...
    async def reset(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await self.is_allowed(update, context):
            logging.warning(f'Пользователю {update.message.from_user.name} (id: {update.message.from_user.id}) '
                            f'не разрешено сбрасывать разговор')
            await self.send_disallowed_message(update, context)
            return
    ...
    async def transcribe(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.is_group_chat(update) and self.config['ignore_group_transcriptions']:
            logging.info(f'Транскрипция исходит из группового чата, игнорируя...')
            return
        ...
        async def _execute():
            ...
            try:
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    reply_to_message_id=self.get_reply_to_message_id(update),
                    text=f'Не удалось загрузить аудиофайл: {str(e)}. Убедитесь, что файл не слишком большой. (макс. 20 MB)',
                    parse_mode=constants.ParseMode.MARKDOWN
                )
                return
            ...
            try:
                ...
                logging.info(f'Новый запрос на расшифровку получен от пользователя {update.message.from_user.name} '
                             f'(id: {update.message.from_user.id})')
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logging.info(
            f'Получено новое сообщение от пользователя {update.message.from_user.name} (id: {update.message.from_user.id})')
        chat_id = update.effective_chat.id
        user_id = update.message.from_user.id
        ...
        if self.is_group_chat(update):
            ...
            if prompt.lower().startswith(trigger_keyword.lower()):
                ...
                if update.message.reply_to_message and update.message.reply_to_message.from_user.id == context.bot.id:
                    logging.info('Сообщение — это ответ боту, позволяю...')
                else:
                    logging.warning('Сообщение не начинается с триггерного ключевого слова, игнорирую...')
                    return
        ...
        if self.is_group_chat(update):
            ...
            for user in itertools.chain(allowed_user_ids, admin_user_ids):
                if await self.is_user_in_group(update, context, user):
                    ...
                    return True
            logging.info(f'Сообщения в групповом чате от пользователя {update.message.from_user.name} '
    ...
    async def check_allowed_and_within_budget(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        if not await self.is_allowed(update, context):
            logging.warning(f'Пользователю {update.message.from_user.name} (id: {update.message.from_user.id}) '
                            f'запрещено использовать бота')
            await self.send_disallowed_message(update, context)
            return False

        if not await self.is_within_budget(update, context):
            logging.warning(f'Пользователь {update.message.from_user.name} (id: {update.message.from_user.id}) '
                            f'достиг предела использования.')
